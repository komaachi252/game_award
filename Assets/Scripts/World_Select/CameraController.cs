...
public class CameraController: MonoBehaviour
{
    ...
    private const float WORLD_EARTH_RANGE = 4.0f;//地球とカメラの距離　ワールドバージョン
    private const float STAGE_EARTH_RANGE = 2.0f;//地球とカメラの距離　ステージバージョン
    private const float SELECT_RANGE = (WORLD_EARTH_RANGE - STAGE_EARTH_RANGE);//選択移動の距離

    private const float WORLD_MOVE_FLAME = 15.0f;//ワールド移動フレーム数
    ...
    void Update()
    {
        ...
        if (select_flag == 0)//ワールド選択の時
        {
            ...
            if (flag == 0 && now_world != next_world)//現在選択ワールドと次に選択してるワールドが違ったら
            {
            ...
            if (flag == 5)
            {
                select_flame = SELECT_RANGE / SELECT_MOVE_FLAME;//一回で移動する距離
                select_flame = select_flame * Time.deltaTime;
                flag = 6;
            }
            ...
            if (flag == 6)
            {
                ...
                earth_range = earth_range + select_flame;//移動

                
                if (next_world != now_world)//ワールド選択に移動中にステージが切り替わったら
                {
                    float kakudo;//目標までの角度
                    kakudo = Vector3.Angle(this.transform.position - earth.position, world_marking[next_world].position - earth.position);
                    move_flame = kakudo / WORLD_MOVE_FLAME;//一回の移動量
